...
const URL = "./models/my-pose-model_2/";
...
async function predictAndDraw() {
    ...
    for (let i = 0; i < maxPredictions; i++) {
        ...
        if (prediction[i].probability > 0.8) {
            if (!predictionHistory[prediction[i].className]) {
                predictionHistory[prediction[i].className] = { count: 0, timestamp: Date.now() };
            } else {
                predictionHistory[prediction[i].className].count++;
                if (Date.now() - predictionHistory[prediction[i].className].timestamp >= 3000) {
                    writeNumber(prediction[i].className);
                    predictionHistory[prediction[i].className] = { count: 0, timestamp: Date.now() };
                }
            }
        } else {
